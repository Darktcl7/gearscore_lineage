...
# DATA FROM item_extras.py MAPPING
mapping = {
    # ============================================
    # LEGENDARY CLASSES
    # ============================================
...
def determine_type(name):
    ...
    if 'helmet' in lower_name or 'circlet' in lower_name or 'helm' in lower_name: return 'Helmet'
    if 'gloves' in lower_name or 'gauntlets' in lower_name or 'jarngreipr' in lower_name: return 'Gloves'
    if 'boots' in lower_name: return 'Boots'
    if 'gaiters' in lower_name: return 'Pants'
    if 'breastplate' in lower_name or 'robe' in lower_name or 'armor' in lower_name or 'tunic' in lower_name: return 'Torso'
    if 'cloak' in lower_name or 'wing' in lower_name or 'scale' in lower_name: return 'Cloak'
    if 'sigil' in lower_name or 'heart' in lower_name or 'shell' in lower_name or 'horn' in lower_name: return 'Sigil'
    if 'necklace' in lower_name: return 'Necklace'
    if 'ring' in lower_name: return 'Ring'
    if 'belt' in lower_name: return 'Belt'
    if 'orb' in lower_name or 'bow' in lower_name or 'skull' in lower_name or 'hand' in lower_name: return 'Weapon'
    # Fallback to check specific list headers (not possible here easily, but names are descriptive)
    
    # Check Legendary/Skills
    if 'classcard' in mapping[name] or 'Legendary' in name: return 'Legendary Class'
    if 'agathion' in mapping[name].lower() or 'Zarich' in name: return 'Legendary Agathion'
    if 'book' in mapping[name].lower() or 'increase' in lower_name: return 'Inheritor Book'

    return 'Other'

print(f"Starting population of {len(mapping)} items...")
...
for name, icon in mapping.items():
    ...
    if Item.objects.filter(name=name).exists():
        # Update icon if missing? Or just skip
    ...
    Item.objects.create(
        name=name,
        icon_file="choices/" + icon, # Prefixing with choices/ folder as that's where they are
        item_type=item_type,
        # Default stats
        attack_power=0,
        defense_power=0
    )
    print(f"Created: {name} [{item_type}]")
    created_count += 1

print(f"Done! Created {created_count} items. Skipped {skipped_count}.")

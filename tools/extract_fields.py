...
# Pattern to extract field name and label
# <label for="id_uron_char_mag" class="form-label">Magic Damage</label>
pattern = r'<label for="id_(\w+)" class="form-label">([^<]+)</label>'

print("class CharacteristicsStats(models.Model):")
print("    character = models.OneToOneField(Character, on_delete=models.CASCADE, related_name='characteristics_stats')")
print("")

matches = re.findall(pattern, html_content)
for name, label in matches:
    print(f'    {name} = models.IntegerField("{label}", default=0)')

print("")
print("    def __str__(self):")
print("        return f\"Characteristics for {self.character.name}\"")
